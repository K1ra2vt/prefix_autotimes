...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, required=True, default='AutoTimes_Gpt2',
                        help='model name, options: [AutoTimes_Llama, AutoTimes_Gpt2, AutoTimes_Opt1b]')  # 模型选择
    ...
    parser.add_argument('--llm_ckp_dir', type=str, default='/home/u4_3090_4/baseModel_gpt2', help='llm checkpoints dir')
    ...
    # 前缀部分的内容，idea1
    parser.add_argument('--use_prefix', action='store_true', help='use conditional prefix', default=True)
    parser.add_argument('--prefix_injection_mode', type=str, default='deep', help='prefix injection mode: shallow (prompt tuning) or deep (K/V injection)')
    parser.add_argument('--prefix_length', type=int, default=4, help='prefix length')
    parser.add_argument('--prefix_mlp_hidden', type=int, default=512, help='prefix mlp hidden dim')
    parser.add_argument('--prefix_mlp_layers', type=int, default=2, help='prefix mlp layers')
    parser.add_argument('--freeze_llm_except_prefix', action='store_true', help='freeze LLM except prefix params', default=True)
    parser.add_argument('--max_prompt_length', type=int, default=128, help='maximum length for structured prompts')
    parser.add_argument('--prefix_learning_rate', type=float, default=None, help='learning rate for prefix parameters')
    parser.add_argument('--init_with_zeros', action='store_true', help='initialize prefix mlp with zeros', default=False)
    parser.add_argument('--use_lora_layerwise_prefix', action='store_true', help='use LoRA + Layer-wise enhanced prefix', default=False)
    parser.add_argument('--lora_rank', type=int, default=8, help='LoRA rank for low-rank adaptation')
    parser.add_argument('--lora_alpha', type=int, default=16, help='LoRA alpha scaling factor')
    parser.add_argument('--num_layer_groups', type=int, default=3, help='number of layer groups for layer-wise prefix')
    parser.add_argument('--layerwise_prefix_lengths', type=int, nargs='+', default=None, help='prefix lengths for each layer group, e.g., 2 4 6')
    
    # 自适应选择性注入Prefix参数
    parser.add_argument('--use_adaptive_prefix', action='store_true', help='use adaptive selective prefix injection', default=False)
    parser.add_argument('--adaptive_use_static_gating', action='store_true', help='use learnable static layer gating', default=True)
    parser.add_argument('--adaptive_use_dynamic_intensity', action='store_true', help='use dynamic intensity adjustment', default=True)
    parser.add_argument('--adaptive_min_gate_value', type=float, default=0.1, help='minimum gate value to prevent complete shutdown')
    parser.add_argument('--adaptive_gate_temperature', type=float, default=0.1, help='temperature for gate sharpness (lower = sharper)')
    ...
    parser.add_argument('--encoder_decoder_learning_rate', type=float, default=None, help='learning rate for encoder/decoder parameters')
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            if args.use_prefix:
                setting = '{}_{}_{}_{}_sl{}_ll{}_tl{}_lr{}_bt{}_wd{}_hd{}_hl{}_cos{}_mix{}_pl{}_pmh{}_pml{}_mpl{}_{}_{}'.format(
                    args.task_name,
                    args.model_id,
                    args.model,
                    args.data,
                    ...
                    args.mix_embeds,
                    args.prefix_length,
                    args.prefix_mlp_hidden,
                    args.prefix_mlp_layers,
