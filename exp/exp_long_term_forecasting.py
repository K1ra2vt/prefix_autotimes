...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _select_optimizer(self):
        # Group parameters by module type for different learning rates
        prefix_params = []
        ...
        for n, p in self.model.named_parameters():
            ...
            elif n.startswith('encoder.') or n.startswith('decoder.'):
                encoder_decoder_params.append(p)
            else:
        ...
        # Prefix parameters - use prefix_learning_rate if specified, otherwise use default learning_rate
        if prefix_params:
            prefix_lr = self.args.prefix_learning_rate if self.args.prefix_learning_rate is not None else self.args.learning_rate
            param_groups.append({
                'params': prefix_params,
                'lr': prefix_lr,
                'initial_lr': prefix_lr,
                'weight_decay': self.args.weight_decay,
                'name': 'prefix'
            })
            if (self.args.use_multi_gpu and self.args.local_rank == 0) or not self.args.use_multi_gpu:
                print(f'Prefix learning rate: {prefix_lr}')

        # Encoder/Decoder parameters - use encoder_decoder_learning_rate if specified, otherwise use default learning_rate
        if encoder_decoder_params:
            enc_dec_lr = self.args.encoder_decoder_learning_rate if self.args.encoder_decoder_learning_rate is not None else self.args.learning_rate
            param_groups.append({
                'params': encoder_decoder_params,
                'lr': enc_dec_lr,
                'initial_lr': enc_dec_lr,
                'weight_decay': self.args.weight_decay,
                'name': 'encoder_decoder'
            })
            if (self.args.use_multi_gpu and self.args.local_rank == 0) or not self.args.use_multi_gpu:
                print(f'Encoder/Decoder learning rate: {enc_dec_lr}')

        # Other parameters - use default learning_rate
        if other_params:
            param_groups.append({
                'params': other_params,
        ...
        if (self.args.use_multi_gpu and self.args.local_rank == 0) or not self.args.use_multi_gpu:
            print(f'Default learning rate: {self.args.learning_rate}')
        return model_optim
    ...
    def vali(self, vali_data, vali_loader, criterion, is_test=False):
        ...
        with torch.no_grad():
            for i, batch_data in enumerate(vali_loader):
                ...
                if len(batch_data) == 5:  # with context_features
                    batch_x, batch_y, batch_x_mark, batch_y_mark, batch_context_features = batch_data
                    # 将字典中的tensor值移到设备上
                    if batch_context_features is not None and isinstance(batch_context_features, dict):
                        batch_context_features = {k: v.to(self.device) if isinstance(v, torch.Tensor) else v 
                                                for k, v in batch_context_features.items()}
                else:  # without context_features
                    batch_x, batch_y, batch_x_mark, batch_y_mark = batch_data
                    batch_context_features = None
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        outputs = self.model(batch_x, batch_x_mark, None, batch_y_mark, context_features=batch_context_features)
                else:
                        outputs = self.model(batch_x, batch_x_mark, None, batch_y_mark, context_features=batch_context_features)
                if is_test:
            ...
            if self.args.cosine:
                ...
                if (self.args.use_multi_gpu and self.args.local_rank == 0) or not self.args.use_multi_gpu:
                    ...
                    for i, group in enumerate(model_optim.param_groups):
                        ...
                        print(f"{group_name} lr = {group['lr']:.10f}")
            else:
