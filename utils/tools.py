...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    if epoch in lr_adjust.keys():
        ...
        for param_group in optimizer.param_groups:
            # Use the base learning rate adjustment but scale by each group's initial lr
            initial_lr = param_group.get('initial_lr', param_group['lr'])
            param_group['lr'] = base_lr * (initial_lr / args.learning_rate) if args.learning_rate > 0 else base_lr
        if (args.use_multi_gpu and args.local_rank == 0) or not args.use_multi_gpu:
            print('learning rates adjusted based on base_lr = {}'.format(base_lr))
